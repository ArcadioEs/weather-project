...
function usage() {
    echo "Build your example/weather-project docker image."
    echo "Usage:"
    echo "   $0 [-h] [-q] [-t]"
    echo
    echo "Flags:"
    ...
    echo "   -t        tag, tag the image e.g:registry-name/image-name:tag-version, default:weather-project:latest"
    echo "Example:"
    echo "   ./build.sh"
    echo "   ./build.sh -t registry:5000/example/weather-project:0.0.1"
    echo "   ./build.sh -h"
    echo "   ./build.sh -q"
}
